...
const updatePage = async () => {
  ...
  const protein = fruitsArray.filter(checkProtein);

  let names = protein.map(function(element){
    return `${element.name}`;
})
...
function checkProtein(fruit) {
  // console.log(fruit.family);
  return fruit.nutritions.protein > 0;
}
